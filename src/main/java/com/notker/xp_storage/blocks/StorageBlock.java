...
public class StorageBlock extends BlockWithEntity implements BlockEntityProvider, Waterloggable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return this.getDefaultState().with(Properties.HORIZONTAL_FACING,
                ctx.getPlayerLookDirection().getOpposite()).with(Properties.WATERLOGGED, bl);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            //if (isSurvival) {
                ...
                // Sculk Block
                if (mainHand.isOf(Items.SCULK)) {
                    return insertSculkBlock(itemCountInHand, world, pos, player, hand, tile);
                }
    ...
    private ActionResult insertSculkBlock(int itemCount, World world, BlockPos pos, PlayerEntity player, Hand hand, StorageBlockEntity tile) {

        try (Transaction transaction = Transaction.openOuter()) {
            tile.liquidXp.insert(FluidVariant.of(ModFluids.LIQUID_XP), itemCount * XpStorage.MB_PER_XP, transaction);
            world.playSound(null, pos, SoundEvents.BLOCK_SCULK_BREAK, SoundCategory.BLOCKS, 1f, 1f);
    ...
    private ActionResult containerXpToPlayer(int itemCount, World world, BlockPos pos, PlayerEntity player, StorageBlockEntity tile) {
        if (tile.liquidXp.amount > 0) {
            ...
            world.playSound(null, pos, SoundEvents.BLOCK_RESPAWN_ANCHOR_DEPLETE.value(), SoundCategory.BLOCKS, 0.125f, 1f);
        }
        tile.isAuthPlayer = false;
        return ActionResult.SUCCESS;
    }
